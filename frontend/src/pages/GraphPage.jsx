...
function ActivityHeatmap({ items }) {
  ...
  useEffect(() => {
    ...
    const today   = dayjs();
    const start   = today.subtract(83, 'day');   // 12 weeks
    const days    = [];
    for (let i = 0; i <= 83; i++) {
...
function SavesChart({ items }) {
  ...
  useEffect(() => {
    ...
    // Build daily saves for last 30 days
    const today = dayjs();
...
export default function InsightsPage() {
  ...
  // Top tags formatted for D3
  const topTags = (tags || [])
    .map(t => typeof t === 'string' ? { name: t, count: items.filter(i => i.tags?.includes(t)).length } : t)
    .filter(t => t.count > 0)
    .sort((a, b) => b.count - a.count)
    .slice(0, 10);
  ...
  const statCards = [
    ...
    { label: "Tags Used",      value: topTags.length,                         icon: Tag,      color: "#38bdf8" },
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <motion.div {...fadeUp(0.3)} className="lg:col-span-2 card p-6"
          ...
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex items-center gap-1.5 text-[10px] text-[var(--text-3)]">
              ...
              <span>Less</span>
              {[0.2, 0.5, 0.8, 1].map(o => (
                <div key={o} className="w-2.5 h-2.5 rounded-sm" style={{ background: `rgba(245,158,11,${o})` }} />
              ))}
              <span>More</span>
            </div>
          </div>
        ...
        <motion.div {...fadeUp(0.35)} className="card p-6"
          ...
          {loading
            ...
            : topTags.length > 0
              ? <TagBars tags={topTags} />
              : <p className="text-[var(--text-3)] text-sm text-center py-10">No tags yet — AI will add them as you save content</p>
          }
        </motion.div>

        {/* Content type cards row */}
        <motion.div {...fadeUp(0.4)} className="lg:col-span-3">
          <h2 className="section-heading text-sm mb-4 flex items-center gap-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
            {Object.entries(TYPE_COLOR).map(([type, color]) => {
              ...
              const pct   = items.length ? Math.round((count / items.length) * 100) : 0;
              return (
                <motion.div key={type} whileHover={{ y: -3 }}
                  className="card p-4 flex flex-col items-center gap-2 text-center cursor-default"
                  style={{ background: `${color}08`, borderColor: `${color}20` }}>
                  <div className="w-8 h-8 rounded-xl flex items-center justify-center"
                    style={{ background: `${color}18` }}>
                    <Icon size={16} style={{ color }} />
                  </div>
                  <div className="text-lg font-black text-white leading-none" style={{ fontFamily: 'Syne' }}>
                    {count}
                  </div>
                  <div className="text-[10px] capitalize" style={{ color: `${color}bb` }}>{type}</div>
